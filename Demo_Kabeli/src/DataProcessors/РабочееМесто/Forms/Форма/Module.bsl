...
Процедура НомерПППриИзменении(Элемент)
	...
	УстановитьОтборСписка("НомерПП", НомерПП);	
	
	Если Не ЗначениеЗаполнено(НомерПП) Тогда
		УдалитьОтборСписка("НомерПП");	
	КонецЕсли;				
...
Процедура ЗавершенныеПриИзменении(Элемент)
	...
	Если Завершенные Тогда  
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыЖруналКабелей.Подписан");
	Иначе
		Статус = Неопределено;
	КонецЕсли;
