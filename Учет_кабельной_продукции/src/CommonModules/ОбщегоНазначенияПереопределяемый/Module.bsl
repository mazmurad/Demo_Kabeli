...
#Область МазановМ    
Функция СписокУдаляемыхСтрокКабельныеЛинии(Ссылка, КабельныеЛинии) Экспорт   
	
	КабельныеЛинииДо    = Ссылка.КабельныеЛинии;
	КабельныеЛинииПосле = КабельныеЛинии;
	
	УдаляемыеКабельныеЛинии = Новый Массив;
	Для Каждого ТекСтрокаДо Из КабельныеЛинииДо Цикл  
		СтрокаПосле = КабельныеЛинииПосле.Найти(ТекСтрокаДо.КабельнаяЛиния,  "КабельнаяЛиния"); 
		Если СтрокаПосле = Неопределено Тогда  
			УдаляемыеКабельныеЛинии.Добавить(ТекСтрокаДо.КабельнаяЛиния);
		КонецЕсли;	
	КонецЦикла;	
...
Функция ПроверитьОбъектУникален(Ссылка, Наименование, ИмяОбъекта, Отказ) Экспорт

	Если Не НаименованиеОбъектаУникально(Ссылка, Наименование, ИмяОбъекта) Тогда    
		ТекстОшибки = СтрШаблон("%1 уже есть в базе!", Наименование);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,,, Отказ);	 
		ЗаписьЖурналаРегистрации(СтрШаблон("Создать %1 кабеля", ИмяОбъекта), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки); 		
	КонецЕсли;
...
Функция НаименованиеОбъектаУникально(СсылкаНаТекОбъект, Наименование, ИмяОбъекта) Экспорт

	УстановитьПривилегированныйРежим(Истина);
	Запрос = Новый Запрос;            
	Запрос.УстановитьПараметр("Наименование", СокрЛП(ВРег(Наименование)));   
	Запрос.УстановитьПараметр("ТекОбъект",    СсылкаНаТекОбъект);   	
	Запрос.Текст = "ВЫБРАТЬ
	               |	ОбъектПроверки.Наименование КАК Наименование
	               |ИЗ
	               |	Справочник.ИмяОбъекта КАК ОбъектПроверки
	               |ГДЕ
	               |	ВРЕГ(ОбъектПроверки.Наименование) = &Наименование
				   |	И ОбъектПроверки.Ссылка <> &ТекОбъект";
...
Функция СтрокаБезЦифр(ТекущаяСтрока) Экспорт
	...
	СтрокаБезЦифр = ТекущаяСтрока;
	Для каждого ТекЦифра Из Цифры Цикл		
		СтрокаБезЦифр = СтрЗаменить(СтрокаБезЦифр, ТекЦифра, ""); 	
	КонецЦикла;
...
Функция СтрокаБезСимволов(ТекущаяСтрока) Экспорт
	...
	СтрокаБез = ТекущаяСтрока;
	Для каждого ТекЦифра Из Цифры Цикл		
		СтрокаБез = СтрЗаменить(СтрокаБез, ТекЦифра, ""); 	
	КонецЦикла;
...
Функция ПодрядчикТекущегоПользователя() Экспорт    
	
	ТекПользователь = Пользователи.ТекущийПользователь();
	Подрядчик       = ТекПользователь.Подрядчик;
	Возврат Подрядчик; 
