...
Функция ОтветственныеЛицаОрганизаций(Отбор = Неопределено) Экспорт
	...
	Если ТипЗнч(Отбор) = Тип("Массив") Тогда
		...
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	Подрядчики.Ссылка КАК Организация
		|ИЗ
		|	Справочник.Подрядчик КАК Подрядчики
		|ГДЕ
		|	Подрядчики.Ссылка В(&Отбор)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Подрядчики.Наименование";
...
Функция ДляВсехОрганизацийНазначеныОтветственные(Отбор = Неопределено, Сообщение = "") Экспорт
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоОшибок = 1 Тогда
			...
			СтрокаСообщения = НСтр("ru = 'Не назначен ответственный для подрядчика %1.'");
			
		Иначе
			
			СтрокаСообщения = НСтр("ru = 'Не назначены ответственные для подрядных организаций:
			|%1.'");
